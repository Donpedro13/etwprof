...
WaitableProcessGroup::Size WaitableProcessGroup::GetSize () const
{
    std::lock_guard guard (m_lock);

    return static_cast<Size> (m_processes.size ());
}

WaitableProcessGroup::Size WaitableProcessGroup::GetWaitingSize () const
{
    std::lock_guard guard (m_lock);
...
WaitableProcessGroup::Size WaitableProcessGroup::GetFinishedSize () const
{
    return GetSize () - GetWaitingSize ();
}
